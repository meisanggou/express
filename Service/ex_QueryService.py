...
def bind():
    ...
    if "user_name" in request_data and "openid" in request_data:
        ...
        return jsonify({"status": 001, "message": "query success", "data": user})
    else:
        return jsonify({"status": 400})
    ...
    return jsonify({"status": 001, "message": "bind success", "data": {"old": old_user["user_name"], "new": user_name}})
...
def explain_express():
    try:
        ...
        com = infos[0]
        if len(com) <= 0:
            return jsonify({"status": 421, "message": u"快递公司不明确"})
        # 查询快递是否存在
        com_info = eDB.select_com(com)
        if len(com_info) == 0:
            return jsonify({"status": 421, "message": u"快递公司不支持"})
        if len(com_info) > 1:
            return jsonify({"status": 421, "message": u"快递公司不明确"})
        ...
        if check_result is False:
            return jsonify({"status": 421, "message": message})
        check_result = eDB.check_listen_record(com_code, waybill_num, user_no)
        if check_result is True:
            return jsonify({"status": 421, "message": u"已经在监听"})
        remark = ""
        if len(infos) >= 3:
            remark = infos[2][:10]
        ...
        return jsonify({"status": 001, "message": "check success", "data": data})
    except Exception as e:
        return jsonify({"status": 500, "message": str(e.args)})
...
def add_listen():
    ...
    return jsonify({"status": 001, "message": "listen success", "data": listen_info})
...
def look_listen():
    ...
    if len(listen_info) <= 0:
        return jsonify({"status": 423, "message": u"查找的快递编号不存在"})
    ...
    return jsonify({"status": 001, "message": "listen success", "data": {"express_info": listen_info,
...
def get_com():
    ...
    return jsonify({"status": 001, "message": "get success", "data": com_info})
...
def mine_express():
    ...
    return jsonify({"status": 001, "message": "get success", "data": listen_info})
